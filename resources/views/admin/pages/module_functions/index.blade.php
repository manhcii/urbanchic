        ...
        <tbody>
          @foreach ($modules as $module)
            ...
            <tr class="valign-middle">
              ...
              <td style="width: 100px;">
                <input data-token="{{ csrf_token() }}" data-url="{{ route('admin.loadOrderVeryModel',['table'=>'modules','id'=>$module->id]) }} " class="lb-order text-center"  type="number" min="0" value="{{ $module->iorder?$module->iorder:0 }}" style="width:50px" />
              </td>
              <td>
                  {{ $module->updated_at }}
              </td>
              <td class="wrap-load-active" data-token="{{ csrf_token() }}" data-url="{{ route('admin.loadStatusModule',['id'=>$module->id]) }}">
                  @include('admin.components.load-change-status',['data'=>$module,'type'=>'bản ghi'])
              </td>
              <td>
          ...
          <form action="{{ route(Request::segment(2).'.destroy', $sub->id) }}" method="POST"
            ...
            <tr class="valign-middle bg-gray-light">
              ...
              <td>
                - - - - <input data-token="{{ csrf_token() }}" data-url="{{ route('admin.loadOrderVeryModel',['table'=>'module_functions','id'=>$sub->id]) }} " class="lb-order text-center"  type="number" min="0" value="{{ $sub->iorder?$sub->iorder:0 }}" style="width:50px" />
              </td>
              <td>
                  {{ $sub->updated_at }}
              </td>
              <td class="wrap-load-active" data-token="{{ csrf_token() }}" data-url="{{ route('admin.loadStatusModuleFunction',['id'=>$sub->id]) }}">
