...
<script src="{{ asset('themes/frontend/assets/swiper/swiper.js') }}"></script>
...
<script>
    (function($) {
        ...
        $('.quickview-button').on('click', function(e) {
            ...
            var url = "{{ route('frontend.quickview') }}";
            $.ajax({
                type: "POST",
                url: url,
                data: {
        ...
        $('.load_more').click(function() {
            if ($('.show_content').hasClass('show') == true) {
                ...
                $(this).find('span').html("@lang('Show More')");
            } else {
                $('.show_content').addClass('show');
                $(this).find('span').html("@lang('Hide')")
            }
        });
        ...
        $('.country-select').change(function(e) {
            var _id = $(this).val();
            var city_id = {{ $user_auth->city_id ?? '0' }};
            var i = 0;

            e.preventDefault();
            $.ajax({
                type: "POST",
                url: '{{ route('frontend.order.getship') }}',
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": _id
                },
